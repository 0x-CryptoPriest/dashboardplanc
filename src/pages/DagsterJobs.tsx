...
import { DagsterJob, DagsterRun, Exchange, fetchDagsterJobs, fetchDagsterRuns } from "@/lib/planc-api";
...
function DAGView({ jobs }: { jobs: DagsterJob[] }) {
  const getLayer = (job: DagsterJob): number => {
    ...
    return 1 + Math.max(...job.upstream.map((name) => {
      const parent = jobs.find((item) => item.name === name);
    ...
    <div className="overflow-x-auto">
      <div className="flex gap-8 min-w-[800px] py-4 px-2">
        {layers.map((layer, layerIndex) => (
          <div key={layerIndex} className="flex flex-col gap-3 items-center">
            <div className="text-[10px] text-muted-foreground uppercase tracking-wider mb-1">Layer {layerIndex}</div>
            {layer.map((job) => {
              ...
              return (
                ...
                >
                  ...
                  {job.downstream.length > 0 && <ArrowRight className="absolute -right-6 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />}
                </motion.div>
              );
            })}
          </div>
...
function Content({ exchangeFilter: _exchangeFilter }: { exchangeFilter: Exchange | "all" }) {
  const [jobs, setJobs] = useState<DagsterJob[]>([]);
  const [runs, setRuns] = useState<DagsterRun[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [jobData, runData] = await Promise.all([fetchDagsterJobs(), fetchDagsterRuns()]);
        if (!isMounted) return;
        setJobs(jobData);
        setRuns(runData);
      } catch (err) {
        if (isMounted) {
          setError(err instanceof Error ? err.message : "failed to load dagster data");
        }
      }
    })();
    return () => {
          ...
          <TabsContent value="list" className="space-y-2">
            {jobs.map((job, index) => {
              ...
              return (
                ...
                >
                  <div className="p-3 flex items-center justify-between cursor-pointer hover:bg-muted/30 transition-colors" onClick={() => setExpandedJob(isExpanded ? null : job.id)}>
                  ...
                  {isExpanded && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} className="border-t border-border px-4 py-3 bg-muted/10">
                      ...
                      {job.upstream.length > 0 && <div className="text-xs mt-2"><span className="text-muted-foreground">Upstream: </span><span className="font-mono text-primary">{job.upstream.join(", ")}</span></div>}
                      {job.downstream.length > 0 && <div className="text-xs mt-1"><span className="text-muted-foreground">Downstream: </span><span className="font-mono text-primary">{job.downstream.join(", ")}</span></div>}
                    </motion.div>
                  )}
                </motion.div>
              );
